...
void ServerImpl::OnClientHandler(int client_socket) {
	...
	std::unique_lock<std::mutex> _lock(_mtx);
	_clients_sockets.erase(client_socket);
	close(client_socket);
	if (_clients_sockets.empty() && !running.load())
	{
		_cond.notify_all();
	}
}
...
void ServerImpl::OnRun() {
    ...
    Afina::Concurrency::Executor thread_pool("name", 1, 2, 100, 1000);
    ...
    while (running.load()) {
        ...
        //создать новый поток и обработать клиента
		{
			...
			//th.detach();
			if (thread_pool.Execute(&ServerImpl::OnClientHandler, this, client_socket))
